...
public class SubjectPanel extends JPanel implements ActionListener
{
	...
	public SubjectPanel(StudentMain sm)
	{
		...
		selectdepartmentlabel.setVisible(false);
		...
		this.createtablemodel(sm.s.getDepartmentCode(),sm.s.getSemorYear() );
		
	}
	public SubjectPanel(TeacherMain fm)
	{
		this();
		departmentnamecombo.setVisible(false);
		semoryearcombo.setVisible(false);
		...
		scrollPane.setBounds(10, headerlabel.getY()+headerlabel.getHeight()+20, 1096, this.getHeight()-headerlabel.getHeight()-20);
		headerlabel.setHorizontalAlignment(JLabel.LEFT);
		this.createtablemodel(fm.f.getDepartmentCode(),fm.f.getSemorYear() );
		
	}
	public SubjectPanel(TeacherMain fm,JComponent lastpanel)
	{
		this(fm);
...
//		timer.start();
		...
		departmentnamecombo.setBounds(300, 88, 806, 42);
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(departmentnamecombo.getSelectedIndex()==0 ||semoryearcombo.getSelectedIndex()==0)
		{
			scrollPane.setVisible(false);
		}
			if(e.getSource()==departmentnamecombo)
			{
			 ...
			 if(e.getSource()==semoryearcombo)
			 {
				if(departmentnamecombo.getSelectedIndex()>0 && semoryearcombo.getSelectedIndex()>0)
				{
				 ...
				 this.createtablemodel(Departmentcode[index-1], sem);
			 ...
			 if(e.getSource()==addsubject)
			 {
				 int sem=semoryearcombo.getSelectedIndex();
				 int index=departmentnamecombo.getSelectedIndex();
				 AddSubjectDialog sd=new AddSubjectDialog(Departmentcode[index-1],sem,table);
	...
	public void createtablemodel(String departmentcode,int sem)
	{
		 ResultSet st=new SubjectData().getSubjectinfo(departmentcode,sem);
