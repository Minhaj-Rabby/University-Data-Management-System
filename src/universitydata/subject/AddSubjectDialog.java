 ...
 * Purpose : For adding new subject to department
 */
...
public class AddSubjectDialog extends JDialog implements ActionListener
{
	...
	private JComboBox<String> departmenttypecombo;
	 ...
	 * Create the dialog.
	 */
	public AddSubjectDialog(String departmentCode,int sem,JTable datatable) {
		...
		JLabel lblDepartmentType = new JLabel("Subject Type");
		lblDepartmentType.setForeground(Color.DARK_GRAY);
		lblDepartmentType.setFont(new Font("Segoe UI", Font.PLAIN, 18));
		lblDepartmentType.setBounds(30, 198, 132, 36);
		contentPanel.add(lblDepartmentType);
		...
		JLabel lblPracticalMarks = new JLabel("Other Marks");
		...
		subjectcodefield.setText(new SubjectData().createSubjectcode(Departmentcode, sem));
		...
		contentPanel.add(practicalmarksfield);
		 ...
		 departmenttypecombo = new JComboBox<String>();
		departmenttypecombo.setFont(new Font("Segoe UI", Font.PLAIN, 18));
		departmenttypecombo.setModel(new DefaultComboBoxModel<String>(new String[] {"---select---", "core", "optional"}));
		departmenttypecombo.setFocusable(false);
		departmenttypecombo.setBackground(Color.WHITE);
		departmenttypecombo.setBounds(172, 199, 330, 40);
		contentPanel.add(departmenttypecombo);
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		else if(departmenttypecombo.getSelectedIndex()==0)
		{
			lblError.setVisible(true);
			lblError.setBounds(departmenttypecombo.getX(), departmenttypecombo.getY()+departmenttypecombo.getHeight()-5, 331, 30);
		}
		...
		else if(new SubjectData().isExist(Departmentcode,semoryear,subjectnamefield.getText()))
		{
			lblError.setVisible(true);
			lblError.setBounds(subjectnamefield.getX(), subjectnamefield.getY()+subjectnamefield.getHeight()-5, 331, 30);
		...
		else
		{
			if(e.getSource()==addsubject)
			{
				...
				String subjecttype=(String) departmenttypecombo.getSelectedItem();
				...
				su.setDepartmentCode(Departmentcode);
				...
				if(result==1)
				{
					ResultSet st=new SubjectData().getSubjectinfo(Departmentcode,semoryear);
