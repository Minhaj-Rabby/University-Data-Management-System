...
public class AdminData {
	...
	{	
		int result=0;
			try
			{
				String query="update admin set universityname=?,address=?,emailid=?,contactnumber=?,website=?,lastlogin=?,password=?,logo=?,activestatus=?";
				PreparedStatement pr=con.prepareStatement(query);
				pr.setString(1, a.getUniversityName());
	...
	public Admin getAdminData()
	{
		...
		try
		{
			...
			ResultSet rs=st.executeQuery(query);
			rs.next();
			a.setUniversityName(rs.getString(1));
