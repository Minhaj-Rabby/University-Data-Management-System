...
public class UserData {
	Connection con=DataBaseConnection.getConnection();
			...
			public int addStudentLoginTime(Student s)
			{
				...
				pr.setInt(1, 0);
				pr.setString(2, s.getDepartmentCode());
			...
			public ArrayList<User> getUserInfo(String condition)
			{
				...
				try
				{
					String query="select departmentcode as 'Department',semoryear as 'Sem/Year',userid as 'Userid',logintime as 'Login Time',userprofile as 'User Profile' from users "+condition+" order by sr_no desc";
					Statement st=con.createStatement();
					ResultSet rs=st.executeQuery(query);
					while(rs.next())
					{
						User user=new User();
						user.setDepartmentCode(rs.getString(1));
